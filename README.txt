...
The Algorithm grabs all the data from the input file and processes it line by line. The tabs in the line are replaces by 4 spaces. It checks whether the lines exceeds the page width and if not, then the line is written as it is. If it does, then a function is called that breaks the line into smaller width to fit into page width. In this function, the line checks at the end of page width whether there is a space and keeps on going backwards from that point until a space is encountered. Then the line is written to the output file from start until the last space encountered with spaces at the end and beginning removed. The processes repeated until the all parts of line are broken down and written to the file. Meanwhile, the algorithm also check whether the line has exceeded the page height and it calls a function to print equal signs if it does. 
